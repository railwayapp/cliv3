...
pub async fn connect_subscription_client(
    ...
    let request = format!("wss://backboard.{hostname}/graphql/v2").into_client_request()?;
    ...
    // request
    //     .headers_mut()
    //     .insert("Sec-WebSocket-Version", HeaderValue::from_str("13")?);
    // request.headers_mut().insert(
    //     "Sec-WebSocket-Key",
    //     HeaderValue::from_str("dGhlIHNhbXBsZSBub25jZQ==")?,
    // );
    // request
    //     .headers_mut()
    //     .insert("Origin", HeaderValue::from_str("https://railway.app")?);
    // request
    //     .headers_mut()
    //     .insert("Sec-Fetch-Mode", HeaderValue::from_str("websocket")?);
